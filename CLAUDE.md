...
## Nav bar — ported real 3.0 nav + page cleanup — 2026-07-06
- DECISION (design lead): stop hand-rebuilding the nav (kept diverging on logo/search/colors/sizes); PORT the real DS 3.0 Nav bar into 4.0. Selected = gray (3.0), reverted code NavBar/Sidebar to gray.
- Ported: DS 3.0 "Nav bar" component set (file h6ZML, set 25940:1066, 6 variants) imported by componentKey into 4.0 via importComponentByKeyAsync -> createInstance. New frame "Nav bar - Full (ported from DS 3.0)" (763:374) on Navigation page: 3 Expanded (Rest/Hover-Selected/Focus) + 3 Collapsed, with column/row labels. Faithful: certa gradient logo, Search icon+label, real icons, gray selected, exact sizes. NOTE: these are INSTANCES linked to the DS 3.0 library (dependency) — can detach to localize if 4.0 must be self-contained.
- Removed my inferior hand-built matrix (757:190) and standalone nav. Navigation page cleaned + arranged in a non-overlapping row: _Source Components / Nav bar Documentation / Nav bar Full (ported) / Tabs Documentation.
- OPEN: the CODE kit Sidebar/NavBar (Primitives.jsx) is still my recreation (no gradient logo, boxed search) — does not match the ported 3.0 design beyond gray-selected. If the 3.0 nav is the standard, update the code Sidebar to match (gradient logo asset, search icon+label, sizing) as a follow-up.
...
## Deterministic templates coded — 2026-07-07
- Coded the 3 reference templates 1:1 from the kit in catalyst-ds-4.0/templates/ (HomeDashboard.jsx, VendorsList.jsx, VendorDetail.jsx + home/vendors/detail.html previews). Global-scope screens (window kit), mount to #root, bundle-able by render/ and /design-sync.
- Fixes the "templates rotos" for good — deterministic, not prose-composed: NO emoji (dates = I.Calendar), risk distribution = horizontal bar meters (NOT CircularProgress/Gauge), every KPIStatCard has a delta (+invertDelta on risk), real kit components (Table/Card/Gauge/RAGField/CircularProgress/Tabs/Pagination/Badge/Tag/ProcessStatus/Avatar). All 3 render-verified via render/render.mjs --shot (no runtime errors, 1:1).
- TEMPLATES.md updated: points to these coded templates as canonical; prose spec kept as intent doc. Next /design-sync: render+upload these instead of re-composing from prose (coded = 1:1 by construction).

## Form field rules — audited vs DS 3.0 review alerts — 2026-07-07
- Audited the 4 field alerts on the DS 3.0 "review" page against our 4.0 form fields. 4.0 ALREADY complies (it was built to correct these) — locking them as rules so we do not regress:
- LABEL COLOR (locked): field label is ALWAYS color/text/primary. Never turn the label green/red by success/error state — 3.0 feedback: a label going green on a successful "risk=High" submit confused business users. State lives on the input border + the help/error message, not the label. (Field in Primitives.jsx: label=text-primary, error=separate text-error message. OK.)
