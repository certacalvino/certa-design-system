...
- **Table Cell** (529:94): 7 types, 52px. Entity avatar has 1px border/subtle ring (INSIDE) — separates it from hover/selected row bg (propagates to Table Row). Status badge bg is token-bound + opaque, holds across all row states (not overridden by row bg).
...
- Contrast/polish fixes 2026-06-17: RAG MED tab → Orange/D2 dark text (AA); Table Cell Entity avatar → 1px border/subtle ring; verified Status badge bg holds across row states.
