...
- Split Button (113:792) REBUILT to DS 3.0 outline pattern 2026-06-18 (was filled brand blue → now white/outline). 12 variants Size(S/M) × State(Enabled/Hover/Pressed/Focused/Loading/Disabled). Anatomy: parent HORIZONTAL gap0/pad0, r4, clip, hug both; surface/default bg + border/default 1px all sides; `primary` label section (transparent, hug) + `trigger` chevron section (transparent, FILL height so the divider spans full height). Label = Body Bold 14 text/primary, pad M 8V(md)/16H(xl) · S 6V/12H(lg). Chevron section = 1px border/default LEFT divider only, pad V=label / 8H(md), Chevron-down icon 179:272 @16px text/secondary. States: Hover bg surface/subtle · Pressed bg surface/muted · Focused border/focused 2px · Disabled surface/disabled bg + border/disabled + text/disabled (label+chevron) · Loading = brand spinner arc (ELLIPSE arcData 270°, text/link, 20px) in label, bg=enabled. Heights S=32 / M=36. GOTCHA: no 6px spacing token (8-grid is 2/4/8/12/16…) → S vertical padding 6 is the one raw value; everything else token-bound. Doc showcase fix 2026-06-18: the 12 instances in the Buttons & Actions doc frame (113:793, NONE layout, absolute x) used a fixed 104px column pitch from the old ~80px buttons → after rebuild (102–110w) columns went flush/overlapping; re-laid columns left-aligned with 24px gaps based on widest-per-column (110/Loading 84), S+M rows stay aligned. Component untouched. Source set 113:792 fix 2026-06-18: same overlap inside the component set (96px pitch, NONE layout) — re-laid variants identically (24px column gaps, S row y32 / M row 16px below at y80) and resized the set frame to 818×148 to contain them (no sibling overlap in _Source Components 156:714). Doc header align 2026-06-18: the Split section column-header labels (113:1590–1595 Enabled/Hover/Pressed/Focused/Loading/Disabled) still sat on the old 104px pitch after the column re-lay → re-centered each over its button column (textAlign CENTER, x = colCenter−w/2; centers 201/335/469/603/724/845). GOTCHA: file is in dynamic-page mode — doc-level text nodes don't resolve via sync figma.getNodeById (returns null) nor surface in findAll/children; use figma.getNodeByIdAsync. Node IDs come from get_metadata.
